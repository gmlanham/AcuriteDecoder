...
byte pin = 10; // from 433 receiver, change pin as required for microprocessor.
...
weatherStruct AcuriteDecoder::getWeather(int dataInputPin) {
	//Weather variables
	double realtemp; double realtempF;                                  
	unsigned char hum;
	double windspeed=0;
  ...
  //this first condition test for 4x, synch pulses in series,
    dur = pulseIn(pin, LOW); if ((dur>synchS)&&(dur<synchL)) {
      dur = pulseIn(pin, LOW); if ((dur>synchS)&&(dur<synchL)) { 
        dur = pulseIn(pin, LOW); if ((dur>synchS)&&(dur<synchL)) {
          dur = pulseIn(pin, LOW); if ((dur>synchS)&&(dur<synchL)) {
          }else goto b;
    ...
    for (int i=0; i <= 75; i++){
      dur = pulseIn(pin, LOW); //assign 1 or 0 depending on pulse width
